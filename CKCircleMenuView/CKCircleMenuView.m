...
@property (nonatomic) NSArray* arrayImageSelected;
...
- (id)initAtOrigin:(CGPoint)aPoint usingOptions:(NSDictionary *)anOptionsDictionary withImageArray:(NSArray *)anImageArray selectedArray:(NSArray *)arraySelected{
	_arrayImageSelected = arraySelected;
	_arrayImage = anImageArray;
...
- (UIView*)createButtonViewWithImage:(UIImage*)anImage andTag:(int)aTag andTitle:(NSString*)aTitle
{
	...
	UIImage* img = self.arrayImageSelected[aTag-1];
	[tButton setImage:img forState:UIControlStateSelected];
...
- (void)resetButtonState
{
	for (int i = 1; i <= self.buttons.count; i++) {
		...
		if (_arrayImageSelected.count > 0 && i != self.hoverTag){
			UIButton *btn = [self viewWithTag: i + TAG_BUTTON_OFFSET];
...
- (void)circleButtonTapped:(UIButton*)sender
{
	...
	if (tTag > 0) {
		if (self.delegate && [self.delegate respondsToSelector:@selector(circleMenuActivatedButtonWithIndex:)]) {
			...
			tTag = (tTag - TAG_INNER_VIEW_OFFSET) + TAG_BUTTON_OFFSET;
			UIButton* btn = [self viewWithTag:tTag];
			[btn setSelected:true];
		}
